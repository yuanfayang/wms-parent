package com.deer.wms.project.seed.generator;
...
public class Generator {
    ...
    public static void genCriteria(DatasourceConfigurer configurer, String tableName, String modelName) {
        try {
            ...
            File file = new File(PROJECT_PATH + configurer.getModuleName() + JAVA_PATH + criteriaPathPackage + modelNameUpperCamel + "Params.java");
            if (!file.getParentFile().exists()) {
                file.getParentFile().mkdirs();
            }
            cfg.getTemplate("params.ftl").process(data, new FileWriter(file));

            System.out.println(modelNameUpperCamel + "Params.java 生成成功");
        } catch (Exception e) {
            throw new RuntimeException("生成Params失败", e);
        }
