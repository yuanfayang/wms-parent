...
public class LoginServiceImpl implements LoginService {
	...
	/* (non-Javadoc)
	 * @see com.deer.wms.system.manage.service.LoginService#loginVerify(com.deer.wms.system.manage.model.user.UserLogin)
	 */
	@Override
	@Transactional
	public UserDetail loginVerify(UserLogin login) {
		//验证账号和密码是否正确，以及账号状态是否正常，通过验证后获取登录的账号信息
        UserInfo userInfo = userInfoService.validate(login.getAccount(), login.getPassword());
        ...
        detail.setMenus(permissionService.findPermissionMenuByUserId(detail.getUserId()));
        detail.setUrls(permissionService.findPermissionUrlByUserId(detail.getUserId()));
        detail.setRoutes(permissionService.findPermissionRoutesByUserId(detail.getUserId()));
