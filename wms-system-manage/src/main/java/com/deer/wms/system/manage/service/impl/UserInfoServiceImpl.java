...
public class UserInfoServiceImpl extends AbstractService<UserInfo, Integer> implements UserInfoService {
	...
	@Override
	public List<UserListVO> findUserFormListNew(UserInfoParams params) {
		return userInfoMapper.selectUserFormListNew(params);
	}
...
//		List<UserInfo> users = userInfoMapper.select(userInfo);
	...
	public void modifyUserInfo(UserInfoModify modify, CurrentUser currentUser) {
		...
		super.update(userInfo);
		// 添加用户角色信息
		if (null != modify.getRoleId()) {
			userRoleService.updateUserRole(userInfo.getUserId(), modify.getRoleId());
		}
	}
	...
	/* (non-Javadoc)
	 * @see com.deer.wms.system.manage.service.UserInfoService#pwdReset(com.deer.wms.system.manage.model.user.UserInfo, com.deer.wms.intercept.common.data.CurrentUser)
	 */
	@Override
	public void pwdReset(UserInfo userInfo, CurrentUser currentUser) {
		String salt = RandomUtil.generateString(10);
		userInfo.setPassword(MD5Utils.encryptPassword(userInfo.getAccount(), SystemManageConstant.DEFAULT_PASSWORD, salt));
		userInfo.setSalt(salt);
		userInfo.setModifyUserId(currentUser.getUserId());
		userInfo.setModifyTime(new Date());
		super.update(userInfo);
