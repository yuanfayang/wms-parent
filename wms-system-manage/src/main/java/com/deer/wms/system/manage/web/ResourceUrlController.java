...
import com.deer.wms.system.manage.model.resource.ReourceUrlPo;
...
public class ResourceUrlController {
    ...
    @PostMapping("/getUrlsByResourceId")
    public Result getUrlsByResourceId(@RequestBody ReourceUrlPo criteria) {
        PageHelper.startPage(criteria.getPageNum(), criteria.getPageSize());
        System.out.println("/getUrlsByResourceId");
        System.out.println(criteria.getResourceId());
        List<ResourceUrl> list = resourceUrlService.getUrlsByResourceId(criteria.getResourceId());
        PageInfo pageInfo = new PageInfo(list);
        return ResultGenerator.genSuccessResult(pageInfo);
    }
