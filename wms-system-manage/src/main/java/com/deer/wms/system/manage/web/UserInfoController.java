...
import com.deer.wms.system.manage.model.user.UserInfoParams;
...
public class UserInfoController {
    ...
    @GetMapping()
    @OperateLog(description = "查看用户信息", type = "查询")
     ...
     * @return
     */
    @ApiOperation(value = "分页查账户信息new", notes = "分页获取账户信息列表new")
    @Authority
    @ApiImplicitParams( { @ApiImplicitParam( name = "access-token", value = "token", paramType = "header", dataType = "String", required = true ) } )
    @GetMapping("/list/new")
    @OperateLog(description = "查看用户信息", type = "查询")
    public Result userListNew(UserInfoParams params, @ApiIgnore @User CurrentUser currentUser) {
    	if(currentUser==null){
            return ResultGenerator.genFailResult( CommonCode.SERVICE_ERROR,"未登录错误",null );
        }
        ...
        List<UserListVO> list = userInfoService.findUserFormListNew(params);
        return ResultGenerator.genSuccessResult(new PageInfo<>(list));
    }
    ...
    @ApiImplicitParams( { @ApiImplicitParam( name = "access-token", value = "token", paramType = "header", dataType = "String", required = true ) } )
    @PutMapping("/password")
    @OperateLog(description = "用户修改自己的密码", type = "更新")
    ...
    @Authority
    @GetMapping("/info")
    @OperateLog(description = "获取当前用户信息", type = "查询")
    ...
    @OperateLog(description = "修改用户自己信息", type = "更新")
    @ApiImplicitParams( { @ApiImplicitParam( name = "access-token", value = "token", paramType = "header", dataType = "String", required = true ) } )
    @Authority
    @PostMapping("/info")
    ...
    @ApiImplicitParams( { @ApiImplicitParam( name = "access-token", value = "token", paramType = "header", dataType = "String", required = true ) } )
    @PostMapping
    @OperateLog(description = "新增用户", type = "增加")
    ...
    @ApiImplicitParams( { @ApiImplicitParam( name = "access-token", value = "token", paramType = "header", dataType = "String", required = true ) } )
    @PutMapping("/{userId}")
    @OperateLog(description = "修改当前用户信息", type = "更新")
    ...
    @OperateLog(description = "获取指定账户的详细信息", type = "查询")
    ...
    @OperateLog(description = "启用账户", type = "更新")
    ...
    @OperateLog(description = "停用账户", type = "更新")
    public Result disable(@PathVariable Integer userId, @ApiIgnore @User CurrentUser currentUser) {
        UserInfo userInfo = userInfoService.findById(userId);
        if (userInfo.getAdmin()||userInfo.getAccount().equals(SystemManageConstant.MT_ADMIN_USER)) {
            return ResultGenerator.genSuccessResult(CommonCode.SERVICE_ERROR, "系统管理员帐号不允许操作");
        }
    ...
    @OperateLog(description = "删除账户", type = "删除")
    ...
    @OperateLog(description = "密码重置", type = "更新")
    public Result pwdReset(@PathVariable Integer userId, @ApiIgnore @User CurrentUser currentUser) {
        UserInfo userInfo = userInfoService.findById(userId);
        if (userInfo.getAdmin()) {
            return ResultGenerator.genSuccessResult(CommonCode.SERVICE_ERROR, "系统管理员帐号不允许操作");
        }
